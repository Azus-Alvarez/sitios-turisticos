...
export class SiteService {
   ...
   url = environment.api_sitiosturisticos + 'site/';
   ...
   post(site: Site): Promise<any> {
      return this.http.post(this.url, JSON.stringify(site), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(site: Site): Promise<any> {
      return this.http.put(this.url, JSON.stringify(site), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
