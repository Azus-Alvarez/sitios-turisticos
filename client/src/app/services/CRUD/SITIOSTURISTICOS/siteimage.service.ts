...
export class SiteImageService {
   ...
   url = environment.api_sitiosturisticos + 'siteimage/';
   ...
   post(siteimage: SiteImage): Promise<any> {
      return this.http.post(this.url, JSON.stringify(siteimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(siteimage: SiteImage): Promise<any> {
      return this.http.put(this.url, JSON.stringify(siteimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
