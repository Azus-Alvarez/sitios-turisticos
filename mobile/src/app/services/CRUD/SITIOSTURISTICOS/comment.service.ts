...
export class CommentService {
   ...
   url = environment.api_sitiosturisticos + 'comment/';
   ...
   post(comment: Comment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(comment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(comment: Comment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(comment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
